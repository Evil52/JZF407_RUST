...
#[test]
fn negative_octet() {
    assert_eq!(parse_ipv4("-1.0.0.1"), None);
}
#[test]
fn empty_string() {
...
#[test]
fn alpha_in_ip() {
    assert_eq!(parse_ipv4("192.168.x.1"), None);
}
...
#[test]
fn valid_port() {
    assert_eq!(parse_port("1883"), Some(1883));
}
#[test]
fn max_port() {
...
fn custom_config_round_trip() {
    let cfg = NetworkConfig {
        ...
        broker_ip: [10, 0, 0, 2],
        broker_port: 8883,
        client_id: heapless::String::try_from("factory-ctrl-01").unwrap(),
...
fn credentials_round_trip() {
    let cfg = NetworkConfig {
        mqtt_user: heapless::String::try_from("broker-user").unwrap(),
        mqtt_pass: heapless::String::try_from("s3cr3t-pass").unwrap(),
        web_user: heapless::String::try_from("admin").unwrap(),
        web_pass: heapless::String::try_from("hunter2").unwrap(),
        ..NetworkConfig::default()
    };
...
fn max_length_credentials_round_trip() {
    ...
    let cfg = NetworkConfig {
        mqtt_pass: heapless::String::try_from(full.as_str()).unwrap(),
        ..NetworkConfig::default()
    };
